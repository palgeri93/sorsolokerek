...
def make_sample_workbook_bytes() -> bytes:
    \"\"\"Mintafájl: kizárólag a felhasználó /mnt/data/sample_names.xlsx fájlja.\"\"\"
    default_path = "/mnt/data/sample_names.xlsx"
    if os.path.exists(default_path):
        with open(default_path, "rb") as f:
            return f.read()
    raise FileNotFoundError("Nem található a /mnt/data/sample_names.xlsx. Válaszd a Feltöltött Excel módot és tölts fel egy fájlt.")
...
def read_sheet_dataframe(xls_bytes: bytes, sheet_name: str) -> pd.DataFrame:
    \"\"\"Beolvassa a munkalapot; 'Név' oszlop kötelező (ha nincs, az első oszlopot tekinti névnek).
...
with st.sidebar:
    ...
    if source == "Feltöltött Excel":
        ...
        if uploaded is not None:
            ...
            # opcionális: ha ugyanazon az útvonalon már van fájl, betöltjük
            default_path = "/mnt/data/sample_names.xlsx"
            if os.path.exists(default_path):
                with open(default_path, "rb") as f:
                    xls_bytes = f.read()
                st.caption("Fájl észlelve: /mnt/data/sample_names.xlsx")
            else:
                st.info("Tölts fel egy Excel fájlt, vagy válaszd a Mintafájlt!")
    else:
        try:
            xls_bytes = make_sample_workbook_bytes()
            st.caption("Mintafájl betöltve: a saját /mnt/data/sample_names.xlsx tartalma.")
...
# Válasszuk ki előre a nyertest (Python oldal) – a kiírás JS-ben, csak a végén történik
target_index = None
if spin:
    if len(names) < 1:
        ...
        # naplózás + kizárás (a felirat csak a JS animáció végén jelenik meg)
...
# ---------- Canvas + WebAudio komponens (egy kerék) ----------
def render_wheel_component(names: List[str], target_index: Optional[int] = None, duration_s: float = 3.0, turns: int = 6,
    ...
    if (winnerEl) winnerEl.textContent = ''; // clear previous text
    ...
    if (CONFIG.targetIndex !== null && CONFIG.targetIndex !== undefined) {
      ...
      const finalStart = 270 - targetCenterDeg; // TOP pointer alignment
      ...
      function frame(now) {
        ...
        if (CONFIG.tickSound && audioEnabled && audioCtx) {
          ...
          if (sector !== lastTickSector && t < 1) {
            // tick at each sector transition (before stop)
        ...
        if (t < 1) {
          ...
          if (winnerEl) winnerEl.textContent = `✅ Nyertes: ${names[targetIndex]}`; // only after stop
...
# Egyetlen kerék render (statikus vagy animált)
