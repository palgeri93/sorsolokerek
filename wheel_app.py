...
# Válasszuk ki előre a nyertest (Python oldal, hogy naplót és kizárást tudjunk kezelni)
target_index = None
winner = None
...
if spin:
    if len(names) < 1:
        ...
        # Naplózzunk és jelöljük ki a kizárást a körben
...
def render_wheel_component(names: List[str], target_index: Optional[int] = None, duration_s: float = 3.0, turns: int = 6,
                           audio_enabled: bool = True, tick_sound: bool = True, ding_sound: bool = True, high_fps: bool = True, key_suffix: str = ""):
    payload = {
        "names": names,
        ...
        "highFps": high_fps,
        # a kulcs változása újrarendereli az iframe-et (spin során)
    ...
    function drawWheel(angle=0, highlightIndex=null) {
      ...
      for (let i=0;i<N;i++) {
        ...
        const label = names[i].toString().slice(0,22);
    ...
    if (CONFIG.targetIndex !== null && CONFIG.targetIndex !== undefined) {
      ...
      const finalStart = 90 - targetCenterDeg; // to land with center up
      ...
      function frame(now) {
        ...
        if (CONFIG.tickSound && audioEnabled && audioCtx) {
          // compute which sector under pointer (angle % 360)
          const a = (angle % 360 + 360) % 360;
          const sector = Math.floor(((360 - a + 90) % 360) / arcDeg);
          if (sector !== lastTickSector && t < 1) {
            // tick at each sector transition
        ...
        if (t < 1) {
          ...
          if (CONFIG.dingSound && audioEnabled && audioCtx) playBeep(900, 0.15, 0.1);
        }
      }
      // ensure audio context (may still require button if browser blocks autoplay)
    ...
    """
    components.html(html, height=610, scrolling=False, key=f"wheel_{key_suffix}_{payload['nonce']}")

# Render preview (static) first, so a kerék mindig látszik
...
# Ha sorsolunk, indítsuk az animációt és írjuk ki a nyertest
